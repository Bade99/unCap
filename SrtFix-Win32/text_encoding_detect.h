...
namespace Common
{
	...
	public:
		...
		bool isUTF16(const unsigned char *pBuffer, size_t size); //For now we dont really care whether it is BE or LE
...
bool AutoIt::Common::TextEncodingDetect::isUTF16(const unsigned char * pBuffer, size_t size)
{
	//TODO(fran): if we find more ways to check add them here
	Encoding encoding;
